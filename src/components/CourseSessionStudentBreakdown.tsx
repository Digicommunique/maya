...
export default function CourseSessionStudentBreakdown({
  ...
  const filteredCourses = useMemo(() => {
    if (!searchQuery.trim()) return breakdownData.courseWise;
    const q = searchQuery.toLowerCase();
    return breakdownData.courseWise.filter(c => c.course.toLowerCase().includes(q));
  }, [breakdownData.courseWise, searchQuery]);

  const filteredSessions = useMemo(() => {
    if (!searchQuery.trim()) return breakdownData.sessionWise;
    const q = searchQuery.toLowerCase();
    return breakdownData.sessionWise.filter(s => s.session.toLowerCase().includes(q));
  }, [breakdownData.sessionWise, searchQuery]);
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
            ...
            >
              ...
              <span>Program-wise ({breakdownData.coursesList.length})</span>
            </button>
            <button
            ...
            >
              ...
              <span>Session-wise ({breakdownData.sessionsList.length})</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 p-4 rounded-xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-slate-900">{breakdownData.coursesList.length}</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-100 p-4 rounded-xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold text-slate-900">{breakdownData.sessionsList.length}</p>
          </div>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead>
                <tr className="bg-slate-900 text-white text-xs">
                  ...
                  {breakdownData.sessionsList.map(session => (
              ...
              <tbody className="divide-y divide-slate-100 text-xs">
                {breakdownData.coursesList.map((course, idx) => {
